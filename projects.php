...
$job_name = $job_number = $street_1 = $street_2 = $city = $state = $zip = $customer_id = $notes = "";
$contact_id_1 = $contact_id_2 = $contact_id_3 = null;
$form_errors = [];
$success_message = "";
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['add_project'])) {
    ...
    // --- MODIFIED: Get contact IDs ---
    $contact_id_1 = !empty($_POST['contact_id_1']) ? (int)$_POST['contact_id_1'] : null;
    $contact_id_2 = !empty($_POST['contact_id_2']) ? (int)$_POST['contact_id_2'] : null;
    $contact_id_3 = !empty($_POST['contact_id_3']) ? (int)$_POST['contact_id_3'] : null;
    ...
    if (empty($form_errors)) {
        // --- MODIFIED: Updated SQL to include new contact ID columns ---
        $sql = "INSERT INTO projects (company_id, job_name, job_number, street_1, street_2, city, state, zip, customer_id, contact_id_1, contact_id_2, contact_id_3, notes, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        if ($stmt = mysqli_prepare($link, $sql)) {
            $default_status = 'Ongoing';
            // --- MODIFIED: Updated bind_param for new columns ---
            mysqli_stmt_bind_param($stmt, "isssssssiiisss", 
                $company_id, $job_name, $job_number, $street_1, $street_2, $city, $state, $zip, 
                $customer_id, $contact_id_1, $contact_id_2, $contact_id_3, $notes, $default_status
            );
                ...
                <div class="<?php echo $is_form_collapsed; ?>" id="addProjectCollapse">
                    <div class="card card-body mb-4">
                        ...
                        <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="POST" class="mt-3">
                            ...
                            <div class="mb-3"><label for="add_customer_id" class="form-label">Customer</label><select class="form-select <?php echo isset($form_errors['customer_id']) ? 'is-invalid' : ''; ?>" id="add_customer_id" name="customer_id" required><option value="">-- Select a Customer --</option><?php foreach ($customers_list as $customer) { echo '<option value="' . $customer['id'] . '"' . ($customer_id == $customer['id'] ? ' selected' : '') . '>' . htmlspecialchars($customer['customer_name']) . '</option>'; } ?></select><div class="invalid-feedback"><?php echo $form_errors['customer_id'] ?? ''; ?></div></div>
                            
                            <!-- *** MODIFIED: Replaced contact_person with three contact dropdowns *** -->
                            <div class="row">
                                <div class="col-md-4 mb-3">
                                    <label for="add_contact_id_1" class="form-label">Primary Contact</label>
                                    <select class="form-select project-contact-dropdown" id="add_contact_id_1" name="contact_id_1" disabled><option value="">-- Select a customer first --</option></select>
                                </div>
                                <div class="col-md-4 mb-3">
                                    <label for="add_contact_id_2" class="form-label">Secondary Contact</label>
                                    <select class="form-select project-contact-dropdown" id="add_contact_id_2" name="contact_id_2" disabled><option value="">-- Select a customer first --</option></select>
                                </div>
                                <div class="col-md-4 mb-3">
                                    <label for="add_contact_id_3" class="form-label">Tertiary Contact</label>
                                    <select class="form-select project-contact-dropdown" id="add_contact_id_3" name="contact_id_3" disabled><option value="">-- Select a customer first --</option></select>
                                </div>
                            </div>
